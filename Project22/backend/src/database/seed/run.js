...
import { resourcesSeed, mediaSeed } from './content.seed.js'
import { storiesSeed, feedbackSeed } from './community.seed.js'
import { helpSeed } from './help.seed.js'
...
async function resetSeedOwnedRecords() {
  ...
  await Promise.all([
    ...
    Feedback.deleteMany({ _id: { $in: feedbackSeed.map(({ _id }) => _id) } }),
    HelpArticle.deleteMany({ slug: { $in: helpSeed.map(({ slug }) => slug) } }),
...
async function seed() {
  ...
  if (!(await QuizVersion.exists({}))) {
    const quizQuestions = await QuizQuestion.find({ active: true }).sort({ order: 1 }).lean()
    await QuizVersion.create({ version: 1, title: 'Interest Quiz', status: 'published', questions: quizQuestions.map(({ _id, version, questionText, type, order, timeLimitSeconds, options }) => ({ questionId: _id, version: version || 1, questionText, type, order, timeLimitSeconds: timeLimitSeconds || 0, options })), scoringVersion: 'domain-weight-v1', createdBy: userIds.superAdmin, publishedAt: new Date() })
  }
  ...
  await Feedback.bulkWrite(toUpserts(feedbackSeed))
  await HelpArticle.bulkWrite(toUpserts(helpSeed))
  ...
  const counts = await Promise.all([
    ...
    Feedback.countDocuments({ _id: { $in: feedbackSeed.map(({ _id }) => _id) } }),
  ])
