...
export function uploadSingleFile({ fieldName = 'file', maxBytes = MAX_FILE_BYTES } = {}) {
  return async (req, _res, next) => {
    ...
    const boundary = /boundary=(?:"([^"]+)"|([^;]+))/i.exec(req.headers['content-type'])?.[1] || /boundary=(?:"([^"]+)"|([^;]+))/i.exec(req.headers['content-type'])?.[2]
    ...
    try {
      ...
      while (cursor !== -1) {
        ...
        if (headerEnd === -1) { cursor = nextMarker; continue }
        ...
        const contentType = headersText.split(/\r\n/).find((line) => line.toLowerCase().startsWith('content-type:'))?.split(':').slice(1).join(':').trim() || 'application/octet-stream'
      ...
      if (!file.content.length || file.content.length > maxBytes) return next(new AppError(413, 'Upload is empty or too large.', 'UPLOAD_TOO_LARGE'))
      ...
      if (!spec.extensions.includes(extension)) return next(new AppError(415, 'The file extension does not match its declared MIME type.', 'INVALID_FILE_EXTENSION'))
      if (!spec.magic(file.content)) return next(new AppError(415, 'The file signature does not match its declared MIME type.', 'INVALID_FILE_SIGNATURE'))

      const safeOriginalName = path.basename(file.filename).replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 180) || `upload.${spec.ext}`
