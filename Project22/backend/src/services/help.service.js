...
export async function listPublishedHelp({ q, category, page = 1, limit = 20, skip = 0 }) {
  ...
  const [articles, total] = await Promise.all([
    HelpArticle.find(filter).sort(q ? { score: { $meta: 'textScore' } } : { sortOrder: 1, title: 1 }).skip(skip).limit(limit).lean(),
...
export async function getPublishedHelp(slugOrId) {
  const filter = /^[a-f0-9]{24}$/i.test(String(slugOrId)) ? { _id: slugOrId, published: true } : { slug: String(slugOrId).toLowerCase(), published: true }
...
export async function listAllHelp({ q, published, page = 1, limit = 20, skip = 0 }) {
  ...
  if (q) filter.$text = { $search: q }
  const [articles, total] = await Promise.all([
    HelpArticle.find(filter).sort({ sortOrder: 1, updatedAt: -1 }).skip(skip).limit(limit).lean(),
    HelpArticle.countDocuments(filter),
  ])
  return { articles, meta: { page, limit, total, pages: Math.ceil(total / limit) } }
}
...
function clean(payload) {
  return {
    ...
    slug: String(payload.slug || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 220),
...
export async function createHelp(adminUserId, payload) {
  ...
  if (!data.title || !data.slug || !data.body) throw new AppError(400, 'title, slug, and body are required.', 'VALIDATION_ERROR')
