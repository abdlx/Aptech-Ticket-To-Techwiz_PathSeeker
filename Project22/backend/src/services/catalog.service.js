...
export async function listDomains() { return Domain.find({ active: true }).sort({ sortOrder: 1, name: 1 }) }
...
const SORTS = { relevance: { title: 1 }, salary: { 'expectedSalary.max': -1 }, growth: { growthRatePercent: -1 } }
...
export async function listCareers({ q, domain, skill, demand, salaryMin, sort = 'relevance', page, limit, skip }) {
  ...
  const [careers, total] = await Promise.all([
    Career.find(filter, projection).sort(sortSpec).skip(skip).limit(limit).populate('domainId', 'name slug').populate('requiredSkills.skillId', 'name slug category'),
...
export async function getCareerBySlug(slug) {
  ...
  const filter = /^[a-f0-9]{24}$/i.test(value) ? { _id: value, ...publishedCareer } : { slug: value.toLowerCase(), ...publishedCareer }
  return Career.findOne(filter).populate('domainId', 'name slug').populate('requiredSkills.skillId', 'name slug category')
}
...
export async function getRelatedCareers(slug, limit = 4) {
  ...
  return Career.find({
    _id: { $ne: career._id },
    $and: [publishedCareer, { $or: [
      { domainId: career.domainId?._id || career.domainId },
      ...(career.tags?.length ? [{ tags: { $in: career.tags.slice(0, 6) } }] : []),
      ...(career.requiredSkills?.length ? [{ 'requiredSkills.skillId': { $in: career.requiredSkills.slice(0, 5).map((item) => item.skillId?._id || item.skillId) } }] : []),
    ] }],
  }).populate('domainId', 'name slug').populate('requiredSkills.skillId', 'name slug category').limit(Math.min(12, Math.max(1, limit)))
}

function editDistance(a, b) {
  const row = Array.from({ length: b.length + 1 }, (_, i) => i)
  for (let i = 1; i <= a.length; i += 1) { let previous = row[0]; row[0] = i; for (let j = 1; j <= b.length; j += 1) { const current = row[j]; row[j] = Math.min(row[j] + 1, row[j - 1] + 1, previous + (a[i - 1] === b[j - 1] ? 0 : 1)); previous = current } }
  return row[b.length]
}
export async function searchSuggestions(q) {
  ...
  const direct = [...careers.map((item) => ({ type: 'career', id: item.slug, label: item.title })), ...skills.map((item) => ({ type: 'skill', id: item.slug, label: item.name }))]
  ...
  return candidates.map((item) => ({ ...item, distance: editDistance(query.toLowerCase(), item.title.toLowerCase()) })).filter((item) => item.distance <= Math.max(2, Math.floor(query.length / 3))).sort((a, b) => a.distance - b.distance).slice(0, 3).map((item) => ({ type: 'correction', id: item.slug, label: item.title, corrected: true }))
}
export async function getRelatedContent(slug, limit = 6) {
  const career = await getCareerBySlug(slug); if (!career) return null
  const tags = (career.tags || []).slice(0, 8)
  const [media, resources] = await Promise.all([
    Multimedia.find({ $and: [publishedContent, { $or: [{ relatedCareerIds: career._id }, ...(tags.length ? [{ tags: { $in: tags } }] : [])] }] }).sort({ ratingAvg: -1, createdAt: -1 }).limit(limit).lean(),
    Resource.find({ ...publishedContent, ...(tags.length ? { tags: { $in: tags } } : {}) }).sort({ downloadCount: -1, createdAt: -1 }).limit(limit).lean(),
  ])
  return { media, resources }
}
export default { listDomains, listSkills, listCareers, getCareerBySlug, getRelatedCareers, getRelatedContent }
