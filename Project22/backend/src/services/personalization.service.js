import { Bookmark, Career, Comparison, RecentlyViewed, SavedFilter, UserProfile, Multimedia, Resource } from '../models/index.js'
...
export async function recordView(userId, { itemType, itemId }) {
  ...
  const Model = (await import('../models/index.js'))[ITEM_TYPE_TO_MODEL[itemType]]
  const exists = await Model.exists({ _id: itemId })
  if (!exists) throw new AppError(404, 'The item you are trying to view does not exist.', 'NOT_FOUND')

  const profile = await UserProfile.findOne({ userId }).select('preferences.activityHistory').lean()
  if (profile?.preferences?.activityHistory === false) return null
...
export async function createComparison(userId, { name, careerIds }) {
  ...
  if (count !== careerIds.length) throw new AppError(400, 'One or more selected careers do not exist or are inactive.', 'VALIDATION_ERROR')
...
export async function exportBookmarks(userId) {
  ...
  const lines = ['PathSeeker — Saved items', '', ...bookmarks.map((bookmark, index) => {
...
export async function getRecommendations(userId, limit = 6) {
  const [profile, latestAttempt, interactions] = await Promise.all([
    ...
    (await import('../models/index.js')).QuizAttempt.findOne({ userId, status: 'completed' }).sort({ completedAt: -1 }).select('topCareerId').lean(),
    Promise.all([
  ...
  const scored = careers.filter((career) => !bookmarkedCareerIds.has(career._id.toString())).map((career) => {
    ...
    const tagMatches = (career.tags || []).filter((tag) => interests.some((interest) => tag.toLowerCase().includes(interest) || interest.includes(tag.toLowerCase()))).length
    ...
    const interactionBonus = bookmarkedCareerIds.has(career._id.toString()) ? -0.05 : viewedCareerIds.has(career._id.toString()) ? 0.03 : 0
    const match = Math.max(1, Math.min(99, Math.round((skillScore * 0.65 + interestScore * 0.2 + demandBonus + quizBonus + interactionBonus) * 100)))
    const reasons = []
  ...
  return scored.sort((a, b) => b.match - a.match || (b.career.growthRatePercent || 0) - (a.career.growthRatePercent || 0)).slice(0, limit)
}
...
export async function getRecommendedContent(userId, limit = 6) {
  ...
  const recent = await RecentlyViewed.find({ userId, itemType: 'career' }).sort({ viewedAt: -1 }).limit(10).select('itemId').lean()
  const careerIds = [...new Set([...bookmarked, ...recent].map((item) => item.itemId.toString()))]
  const careers = careerIds.length ? await Career.find({ _id: { $in: careerIds }, active: true }).select('tags') .lean() : []
  ...
  const [media, resources] = await Promise.all([
    Multimedia.find({ active: true, ...(careerIds.length ? { $or: [{ relatedCareerIds: { $in: careerIds } }, ...(tags.length ? [{ tags: { $in: tags } }] : [])] } : {}) }).sort({ ratingAvg: -1, createdAt: -1 }).limit(limit).lean(),
    Resource.find({ active: true, ...(tags.length ? { tags: { $in: tags } } : {}) }).sort({ downloadCount: -1, createdAt: -1 }).limit(limit).lean(),
  ])
