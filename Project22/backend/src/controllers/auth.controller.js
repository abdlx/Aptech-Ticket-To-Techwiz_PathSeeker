...
export const register = asyncHandler(async (req, res) => {
  const { name, email, password, stage, termsAccepted } = req.body
  ...
  if (termsAccepted !== true) throw new AppError(400, 'You must accept the Terms of Service and Privacy Policy.', 'TERMS_REQUIRED')
  if (!isValidStage(stage)) {
    throw new AppError(400, 'Stage must be one of: student, graduate, professional.', 'VALIDATION_ERROR')
  }

  const user = await authService.registerUser({ name: name.trim(), email, password, stage, termsAccepted })
...
export const login = asyncHandler(async (req, res) => {
  ...
  await issueSessionCookie(res, user, req)
  await logAction(user._id, 'auth.login', 'User', user._id, { ip: req.ip })
...
export const adminLogin = asyncHandler(async (req, res) => {
  ...
  await issueSessionCookie(res, user, req)
  await logAction(user._id, 'auth.admin_login', 'User', user._id, { ip: req.ip })
