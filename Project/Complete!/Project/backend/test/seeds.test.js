...
import { Career, Domain, Feedback, Multimedia, QuizQuestion, Resource, Skill, SuccessStory, User, UserProfile } from '../src/models/index.js'
...
import { resourcesSeed, mediaSeed } from '../src/database/seed/content.seed.js'
import { storiesSeed, feedbackSeed } from '../src/database/seed/community.seed.js'
...
test('Phase 1 seeds are valid, deterministic, and internally referential', async () => {
  ...
  await Promise.all([
    ...
    ...storiesSeed.map((document) => new SuccessStory(document).validate()),
    ...feedbackSeed.map((document) => new Feedback(document).validate()),
