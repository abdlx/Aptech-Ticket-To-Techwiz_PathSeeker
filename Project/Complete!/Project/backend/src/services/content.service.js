...
export async function getResourceById(id) {
  const resource = await Resource.findOne({ _id: id, $or: [{ status: 'published' }, { status: { $exists: false }, active: true }] })
  if (!resource) {
...
export async function getMediaById(id) {
  const media = await Multimedia.findOne({ _id: id, $or: [{ status: 'published' }, { status: { $exists: false }, active: true }] }).populate('relatedCareerIds', 'title slug domainId')
  if (!media) {
...
export async function getRelatedMedia(mediaId, limit = 6) {
  const media = await Multimedia.findOne({ _id: mediaId, $or: [{ status: 'published' }, { status: { $exists: false }, active: true }] }).lean()
  if (!media) throw new AppError(404, 'Media not found.', 'NOT_FOUND')
  const filter = { _id: { $ne: media._id }, $or: [{ status: 'published' }, { status: { $exists: false }, active: true }] }
...
export async function updateMyStory(userId, id, payload) {
  ...
  if (payload.image !== undefined && payload.image?.assetKey && payload.image.assetKey !== story.image?.assetKey) { const previous = story.image; story.image = payload.image; await removeStoryAsset(previous) }
...
export async function submitMyStory(userId, id) {
  ...
  if (!story.authorName || !story.domainId || story.storyText.length < 20) throw new AppError(400, 'Complete the required story fields before submitting.', 'VALIDATION_ERROR')
  if (!['draft', 'changes_requested'].includes(story.status)) throw new AppError(409, 'This story is not awaiting resubmission.', 'STORY_NOT_SUBMITTABLE')
