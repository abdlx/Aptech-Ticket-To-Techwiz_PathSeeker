...
router.get('/health/db', (_req, res) => { const ready = mongoose.connection.readyState === 1; res.status(ready ? 200 : 503).json({ data: { status: ready ? 'ready' : 'not_ready', readyState: mongoose.connection.readyState } }) })
