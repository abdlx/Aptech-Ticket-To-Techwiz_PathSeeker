...
export const submitStory = asyncHandler(async (req, res) => {
  ...
  if (image !== undefined) {
    if (
      !image ||
      !isNonEmptyString(image.assetKey, { min: 10, max: 200 }) ||
      !isNonEmptyString(image.url, { min: 1, max: 2_000 }) ||
      !isSafeHttpUrl(image.url) ||
  ...
  const story = await contentService.submitStory(req.user.id, {
    authorName,
    domainId,
    storyText,
    educationPath,
