...
export function VerifyEmailFlow({ navigate }) {
  ...
  return <AuthFrame navigate={navigate} eyebrow="Email verification" title="Check your inbox" copy="Enter the 6-digit code sent to your email. It expires in 10 minutes.">
    <form onSubmit={submit} noValidate>
      <label>Email address<div className="input-wrap"><Icon name="message" /><input required type="email" placeholder="name@example.com" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></div></label>
...
export function ResetPasswordFlow({ navigate }) {
  ...
  return <AuthFrame navigate={navigate} eyebrow="Create a new password" title={mutation.isSuccess ? 'Password updated' : 'Choose something secure'} copy="Use at least eight characters with uppercase, number, and special characters.">{mutation.isSuccess ? <button className="button primary full" onClick={() => navigate('login')}>Continue to log in</button> : <form onSubmit={submit}><label>New password<div className="input-wrap"><Icon name="lock" /><input required type="password" placeholder="Enter new password (min. 8 characters)" value={password} onChange={(event) => setPassword(event.target.value)} /></div></label><label>Confirm password<div className="input-wrap"><Icon name="lock" /><input required type="password" placeholder="Confirm your new password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></div></label>{!token && <p className="form-error">This reset link is missing its secure token.</p>}<MutationError mutation={mutation} /><button className="button primary full" disabled={!token || mutation.isPending}>{mutation.isPending ? 'Updating…' : 'Update password'} <Icon name="check" /></button></form>}</AuthFrame>
}
...
export function AdminLoginFlow({ navigate }) {
  ...
  return <AuthFrame navigate={navigate} admin eyebrow="PathSeeker administration" title="Welcome back, admin" copy="Sign in with your authorized workspace account."><form onSubmit={(event) => { event.preventDefault(); mutation.mutate({ email, password }) }}><label>Work email<div className="input-wrap"><Icon name="message" /><input required type="email" placeholder="admin@pathseeker.org" value={email} onChange={(event) => setEmail(event.target.value)} /></div></label><label>Password<div className="input-wrap"><Icon name="lock" /><input required type="password" placeholder="Enter admin password" value={password} onChange={(event) => setPassword(event.target.value)} /></div></label><MutationError mutation={mutation} /><button className="button primary full" disabled={mutation.isPending}>{mutation.isPending ? 'Signing in…' : 'Enter admin workspace'} <Icon name="arrow" /></button></form></AuthFrame>
}
