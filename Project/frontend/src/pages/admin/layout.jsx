  ...
  return (
    <div className="admin-layout">
      <aside className="admin-sidebar">
        ...
        <nav><p className="nav-label">Workspace</p>{adminNav.map(([id, label, icon]) => <button key={id} className={screen === id ? 'active' : ''} onClick={() => navigate(id)}><Icon name={icon} /><span>{label}</span>{id === 'admin-feedback' && openFeedback > 0 && <em>{openFeedback}</em>}</button>)}</nav>
        <div className="admin-sidebar-bottom"><button className={screen === 'admin-help' ? 'active' : ''} onClick={() => navigate('admin-help')}><Icon name="help" /> Help center</button><button className={screen === 'admin-settings' ? 'active' : ''} onClick={() => navigate('admin-settings')}><Icon name="settings" /> Settings</button><button onClick={logout}><Icon name="logout" /> Log out</button><div><span className="avatar small">{initials}</span><p><strong>{displayName}</strong><small>{auth.user?.role?.replaceAll('_', ' ') || 'staff'}</small></p><Icon name="more" /></div></div>
      </aside>
