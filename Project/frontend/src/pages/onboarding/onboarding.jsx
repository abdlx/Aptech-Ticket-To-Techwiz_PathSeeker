...
import { useOnboardingDraftStore } from '../../stores/appStores'

export default function OnboardingPage({ navigate }) {
  const { step, setStep, clear } = useOnboardingDraftStore()
  const [selected, setSelected] = useState(['Design', 'Technology'])
  const save = useMutation({ mutationFn: profileApi.updateOnboarding, onError: (error) => toast.error(error.message) })
  const advance = async () => {
    try {
      if (step < 3) {
        await save.mutateAsync({ status: 'in_progress', currentStep: step + 1 })
        setStep(step + 1)
        return
      }
      await profileApi.update({ interests: selected })
      await save.mutateAsync({ status: 'completed', currentStep: 3 })
      clear()
      toast.success('Your Career Passport is ready to build.')
      navigate('quiz')
    } catch {
      // The mutation exposes the actionable error through the toast above.
    }
  }
  const finishLater = async () => {
    try {
      await save.mutateAsync({ status: 'in_progress', currentStep: step })
      navigate('dashboard')
    } catch {
      // Stay on the form so the user can retry without losing the draft.
    }
  }
  ...
  return (
    <div className="onboarding-page">
      <header><Brand /><button disabled={save.isPending} onClick={finishLater}>Save & finish later</button></header>
      ...
      <main>
        <section className="onboarding-card">
          ...
          <div className="onboarding-actions"><button className="button ghost" disabled={step === 1 || save.isPending} onClick={() => setStep(step - 1)}><Icon name="arrowLeft" /> Back</button><button className="button primary" disabled={save.isPending} onClick={advance}>{save.isPending ? 'Saving…' : step < 3 ? 'Continue' : 'Build my passport'} <Icon name="arrow" /></button></div>
        </section>
