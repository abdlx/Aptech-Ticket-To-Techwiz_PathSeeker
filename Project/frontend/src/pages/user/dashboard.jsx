...
function ConnectedDashboardPage({ navigate, onVoice }) {
  ...
  return <div className="dashboard page-stack">
    ...
    <section><SectionHead eyebrow="Personalized for you" title="Your strongest career matches" link={matches.length ? 'See explanations' : undefined} onLink={() => navigate('recommendations')} />{matches.length ? <div className="result-list">{matches.map((match) => <article key={match.careerId._id} className="result-card"><span className={`career-icon ${match.careerId.colorTone || 'lavender'}`}><Icon name={match.careerId.iconKey || 'briefcase'} /></span><div className="result-main"><span className="match-pill">{match.compatibilityScore}% match</span><h3>{match.careerId.title}</h3><p>{match.careerId.summary || match.reasons?.[0]}</p><div className="career-meta"><span><small>Career match</small><strong>{match.compatibilityScore}%</strong></span><span><small>Readiness</small><strong>{match.readinessScore}%</strong></span></div></div><button className="button primary small" onClick={() => navigate('career-detail', match.careerId.slug)}>Explore <Icon name="arrow" /></button></article>)}</div> : <EmptyState title="No matches yet" message="Finish the assessment and PathSeeker will generate explainable recommendations." />}</section>
    ...
    <section><SectionHead eyebrow="Live catalog" title="Trending careers" link="Career Bank" onLink={() => navigate('careers')} /><div className="career-grid">{dashboard.trendingCareers.map((career) => <article className="career-card" key={career._id}><span className={`career-icon ${career.colorTone || 'lavender'}`}><Icon name={career.iconKey || 'briefcase'} /></span><h3>{career.title}</h3><p>{career.domainId?.name} Â· {career.growthRatePercent ?? 0}% projected growth</p><button className="card-link" onClick={() => navigate('career-detail', career.slug)}>View career <Icon name="arrow" /></button></article>)}</div></section>
  </div>
}
